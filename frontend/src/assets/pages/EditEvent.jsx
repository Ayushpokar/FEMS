...
export function EditEvent() {
    const { id } = useParams();
    const [guests, setGuests] = useState([]);   // ✅ separate state
    ...
    useEffect(() => {
        const fetchSingleEvent = async () => {
            try {
                ...
                const [eventRes, guestRes, groupRes] = await Promise.all([
                    axios.get(`/api/getevent/${id}`),
                    axios.get('/api/get_guests'),
                    axios.get('/api/getvolunteer_group')
                ]);


                // const res = await axios.get(`/api/getevent/${id}`)
                const result = eventRes.data;
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div className="flex space-x-2">
                            <select id="guest" name="guest_id" onChange={handleChange} value={String(formData.guest_id)} className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white text-gray-700">
                                {guests?.map((g) => (
                    ...
                    <div>
                        ...
                        <div className="flex space-x-2">
                            <select id="group" name='group_id' value={String(formData.group_id)} onChange={handleChange} className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white text-gray-700">
                                {groups?.map((g) => (
                                    <option key={g.id} value={String(g.id)}>{g.name}</option>
                                ))}
                            </select>
