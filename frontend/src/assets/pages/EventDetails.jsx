...
import { ArrowLeft, CheckCircle, AlertCircle, XCircle, Users, UsersRound, User, UsersIcon, FileText, UserCheck } from 'lucide-react';
...
export function EventDetails() {
    ...
    // output → Mon, 5 January 2025, 02:30 PM
    ...
    const handleStatusUpdate = async (status) => {
        ...
        try {
            await axios.patch(`/api/event/updatestatus/${id}/${status}`, {
                comment: comment
            });
    ...
    return (
        <>
            <div className="flex items-start space-x-4 mb-6">
                <button onClick={() => navigate(-1)} className="flex items-center px-3 py-1.5 border border-gray-300 rounded-md bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 transition-colors mt-1">
            ...
            {role === 'faculty' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-4">
                    <Link to='participate'><EventCard
                        title={"Participate"}
                        value={eventData.total_participants || 0}
                        icon={UsersIcon}
                        iconColor="text-blue-500"
                    /></Link>
                    <EventCard
                        title={"Attendence"}
                        value={eventData.total_attendance || 0}
                        icon={UserCheck}
                        iconColor="text-green-500"
                    />
                    {eventData.current_status == "approved" && (<Link to='report/' ><EventCard
                        title={"Generate"}
                        value={"Report"}
                        icon={FileText}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-gray-900">{formatDateTime(eventData.start_date)}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 mb-1">End Time</p>
                                    <p className="font-medium text-gray-900">{formatDateTime(eventData.end_date)}</p>
                                </div>
                                <div>
                                ...
                                {/* We add 'group relative' so the tooltip knows where to position itself */}
                                <div className="relative group w-max cursor-help">
                                    <p className="text-sm text-gray-500 mb-1">Guest Name</p>

                                    {/* I added a dashed underline so users intuitively know they can hover over it */}
                                    <p className="font-medium text-gray-900 border-b border-dashed border-gray-400 inline-block">
                                        {eventData.guest_name}
                                    </p>

                                    {/* ========================================== */}
                                    {/* THE HIDDEN TOOLTIP CARD                    */}
                                    {/* ========================================== */}
                                    <div className="absolute bottom-full left-0 mb-2 w-64 bg-white border border-gray-200 shadow-xl rounded-xl p-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">

                                        <h4 className="font-bold text-gray-900 mb-1">{eventData.guest_name}</h4>

                                        <p className="text-sm text-gray-700 font-medium">{eventData.guest_designation || 'No Guest'}</p>
                                        <p className="text-xs text-gray-500 mb-3">{eventData.guest_company || '-'}</p>

                                        <div className="border-t border-gray-100 pt-2 mt-2">
                                            <p className="text-xs text-gray-500">
                                                <span className="font-semibold text-gray-700">Email:</span> {eventData.guest_email || 'Not provided'}
                                            </p>
                                            <p className="text-xs text-gray-500">
                                                <span className="font-semibold text-gray-700">Phone:</span> {eventData.guest_mobile || 'Not provided'}
                                            </p>
                                        </div>
                ...
                {role === 'faculty' && eventData?.comment && (
